...
wolf_example_tdfs <- load_discr_data(filename=discr.filename, mix)
...
jags.1 <- run_model(run="short", mix, source, wolf_example_tdfs, model_filename, 
...
jags.2 <- run_model(run="short", mix, source, extra_errors_tdfs, model_filename, 
                    alpha.prior = 1, resid_err, process_err)
...
jags.4 <- run_model(run="short", mix, source, derbridge_tdfs, model_filename, 
                    alpha.prior = 1, resid_err, process_err)
...
jags.4 <- run_model(run="short", mix, source, discr4, model_filename, 
                    alpha.prior = 1, resid_err, process_err)
...
jags.5 <- run_model(run="short", mix, source, tdf_sider, model_filename, 
                    alpha.prior = 1, resid_err, process_err)
...
cat("Model 3: use the fox TDFs\n")
knitr::kable(jags.3$BUGSoutput$summary)

cat("Model 4: SIDER derived TDFs\n")
knitr::kable(jags.4$BUGSoutput$summary)
...
post_df <- dplyr::bind_rows(list(as.data.frame(jags.1$BUGSoutput$sims.matrix), 
                                 ...
                                 as.data.frame(jags.4$BUGSoutput$sims.matrix),
                                 as.data.frame(jags.5$BUGSoutput$sims.matrix))
                            , .id = "Model")
