...
We will take a single pack from the Wolves example in MixSIAR [Semmens et al 2009](https://doi.org/10.1371/journal.pone.0006187) which is based on a study by [Darimont et al 2008](http://dx.doi.org/10.1111/j.1365-2656.2008.01473.x). We run a simple mixing model using the default TDFs included with the example which were described originally by [Roth & Hobson  and compare that with the results obtained using SIDER to estimate them. For this simplified example we use only wolf pack 1 located in region 1 and so omit any factors. The example will work similarly for the full analysis with all packs and all regions included. We run the models with process error (and without residual error: see the MixSIAR documentation for more details).
...
The nearest species to the wolf in the dataset with known TDFs is the fox. While we have an estimate for the means, we still dont have any uncertainty for this, and so will continue to use the "common" errors we used in the previous example. These fox TDFs were used by [Darimont et al 2008](http://dx.doi.org/10.1111/j.1365-2656.2008.01473.x) in a mixing model and were oringally derived from controlled feeding experiments by [Roth & Hobson](https://doi.org/10.1139/z00-008).
...
sd_c <- sd(TDF_est_c$tdf_global)
sd_n <- sd(TDF_est_n$tdf_global)
...
sider_tdf$mu <- matrix(c(rep(mu_c, 3), rep(mu_n, 3)), ncol = 2, nrow = 3,
                       dimnames = list(source$source_names, mix$MU_names))
sider_tdf$sig2 <- matrix(c(rep(sd_c, 3), rep(sd_n, 3)), ncol = 2, nrow = 3,
                       dimnames = list(source$source_names, mix$SIG_names))
