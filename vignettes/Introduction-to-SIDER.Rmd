...
This package estimates Trophic Discrimination Factors (TDF) based on the imputation function within the \href{http://cran.r-project.org/web/packages/MCMCglmm/index.html}{\texttt{MCMCglmm} package} and includes
functionality to include the error associated with building phylogenetic 
trees using the \href{https://github.com/TGuillerme/mulTree}{\texttt{MulTree} package}.
...
It also contains what tissue was used to measure the istopic value and the basic ecology
of the species including whether it is a herbivore etc and whther it is marine or terrestrial.
This data is used to run a model adn impute a value for the new species.
...
The recipeSider function also formats the data for the new species data so that it can be combined with
the data already available within the package using the prepareSider function.
...
With the data formatted as a mulTree object we can decide on a model which will impute the new species estimate. In this case we will run the full model to estimate delta13C with the fixed factors
...
As the function exports the model output to avoid memory issues within R when running over multiple phylogenies make sure you have set the working directory to somewhere appropriate. In this vignette example, we will use the default temporary directory determined by `tempdir()`, but ordinarily, this would be a folder of your own choosing located somewhere sensible on your local machine. We **_strongly_** advise that you do not use this `tempdir` in your own analyses!  In this example, I store the original working directory to `origwd` so that we can return the R session to the orginal working directory after using the temporary one. Then we can finally we can run the analysis using the imputeSider function. This model will normally take 
approximatly 5 minutes, however for brevity we will run a much shorter chain.
...
# NB only for use in this vignette exmaple. You probably do not need to 
