...
#'   
#' @description Runs a \code{\link[MCMCglmm]{MCMCglmm model}} model to impute
#'   delta
#'   
...
#'   Tdf.est and a combined chain in Tdf.global.
#'   
#' @author Kevin Healy
#'   
#' @example ##
...
mulTree.data$random.terms = random.terms
		...
		if((is.null(prior) & mulTree.data$random.term == 
		    "~animal + sp.col + tissue") == TRUE){
...
#mulTree.data$table$animal <-  mulTree.data$table$species
		...
		mulTree(mulTree.data  = mulTree.data , 
		        formula = formula, 
